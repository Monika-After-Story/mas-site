...
var DynaImage = (function($, undefined) {
    ...
    return {
        reInit: function() {
            ...
            $.each($('.dyna-sitebuild-block-img'), function() {
            	var img = $(this).find('img.dyna-sitebuild-block-img-content:not(.noscript-img)');
            	if(isJqueryObjectExists(img)){
            		...
            		if (src.indexOf('?img_id') != -1) {
                        var newSrc = src + '&w=' + w;
                        img.data('src2', newSrc);
                        img.attr('src', newSrc);
                    } else {
                    	img.data('src2', src);
                    	img.attr('src', src);
                    }
            		//img.imageloader({dataattr: 'src2'});
            	}
            });
            //$('.dyna-sitebuild-block-img img.dyna-sitebuild-block-img-content:not(.noscript-img)').imageloader({dataattr: 'src2'});
        }
    }
})(jQuery);
